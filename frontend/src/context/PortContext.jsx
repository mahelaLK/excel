  ...
  const exportTableStyledGen = async (vesselName) => {
    try {
        ...
        gangDetailsGen.gangPlanDetails.forEach((plan) => {
            const details = plan.gangDetails;
            if (details.length===1) {
                const value = Number(details[0][metric]) || 0;
                total += value;
                row.push({
                v: value,
                s: {
        ...
        const firstShift = gangDetailsGen.gangPlanDetails[0].shiftPlanDetails.find(
            (s) => s.ShiftNumber === shiftNo
        );
        const baselineLiftTimes = firstShift?.liftTimePlanDetails ?? [];

        const cumulativeByGang = gangDetailsGen.gangPlanDetails.map(
            (plan, gangIdx) => {
        ...
        baselineLiftTimes.forEach((lt, i) => {
            ...
            gangDetailsGen.gangPlanDetails.forEach((plan, gangIdx) => {
